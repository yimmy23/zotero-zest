...
out = sys.argv[1]; sel = None; theme = None; prefs = False; stats = False; matrix = False
for a in sys.argv[2:]:
    if a == "--dark": theme = 1
    elif a == "--light": theme = 0
    elif a == "--prefs": prefs = True
    elif a == "--stats": stats = True
    elif a == "--matrix": matrix = True
    else: sel = a
pre = ""
post = ""
...
""" % ("[...Services.wm.getEnumerator('zotero:pref')][0]" if prefs else ("[...Services.wm.getEnumerator(null)].find(w=>w.document&&w.document.querySelector('.zest-stats'))" if stats else ("[...Services.wm.getEnumerator(null)].find(w=>w.document&&w.document.querySelector('.zest-matrix'))" if matrix else "Zotero.getMainWindow()")), json.dumps(sel), json.dumps(out)) + post + "return 'saved '+w+'x'+h+' @'+scale;"
print(ev(code))
