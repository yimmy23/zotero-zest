...
- [x] 提交推送、CI成功后发布版本包，最后更新稳定清单并独立核对公开资产。
...
数据保全：现有1个文库、3541条记录、1016篇未删除文献、351条标注，在内存比较元数据/标注与阅读API快照，升级和测试前后一致；没有创建或关闭阅读器标签、没有输出原文数据。临时网络防护拦住2次外部调用，之后恢复；偏好、主题、宽度、当前标签与布局全部恢复，诊断窗口关闭，日常Zotero留在原阅读器。完整证据只保存在上述本地忽略目录。

发布完成：正式最新版本 `v1.4.2` 指向 `9a0c55c02e2eecc607d96646957dedff6e312a1a`，对应 CI `34430748224` 成功。先发布唯一版本资产 `zest.xpi` 并匿名下载验哈希，再更新稳定 `release/update.json` 并从规范公开URL重新下载，版本、地址、Zotero10兼容范围与SHA-512全部和最终冻结包一致。日常安装包同哈希；1.4.1回退包保留，没有发布或修改Refs。
