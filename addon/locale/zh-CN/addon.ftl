...
import-write-failed = 导入未能保存，本次导入的记录均未生效。请解决所报告的问题后重试。{ $error }
...
migrate-report-line = 旧版载体条目：{ $parents } · 笔记：{ $notes } · 解析成功：{ $parsed } · 跳过：{ $skipped } · 文件：{ $files } · 修正页码偏移：{ $offset } · 未能定位的条目：{ $unresolved } · 歧义条目：{ $ambiguous } · 已合并：{ $merged }（{ $hours } 小时）
import-sum-unallocated = 这些记录的页级与日期明细覆盖范围不同，无法准确相加。本条记录未作更改。请使用保留较大值（max）的合并方式，或导入覆盖范围一致的页级和日期明细。
...
anno-copy = 复制文本
anno-action-copy = 复制
anno-card-tip = 点击“定位原文”或双击页码打开标注；正文可选中复制
...
pref-key-save = 保存
pref-key-cleared = 密钥已删除
pref-key-clear-pending = Zest 已停用此密钥，但登录管理器尚未确认删除。请解锁后再次点击保存，以重试删除。
...
menu-citations-update-stale =
    .label = 更新过期的被引数
citations-done = 已更新 { $updated } 条 · 未变 { $unchanged } 条 · 无标识符 { $missing } 条 · 未查到 { $notFound } 条 · 失败 { $failed } 条 · 未改动 { $stopped } 条
...
info-citation-key-copy-failed = 复制失败，可选中文字手动复制
info-citations = 被引数
...
sidebar-stats-sidenav =
    .tooltiptext = Zest · 阅读统计
sidebar-matrix-header =
    .label = Zest · 标注矩阵
sidebar-matrix-sidenav =
    .tooltiptext = Zest · 标注矩阵
sidebar-graph-header =
    .label = Zest · 文库关系图
sidebar-graph-sidenav =
    .tooltiptext = Zest · 文库关系图
sidebar-open-window =
    .tooltiptext = 在独立窗口中打开
sidebar-open-graph =
    .tooltiptext = 在文库下方展开
sidebar-stats-scope = 全部本地阅读记录
sidebar-graph-view = 当前文库视图
sidebar-graph-item = 当前文献及其本地关联文献
sidebar-loading = 正在载入…
sidebar-load-failed = 暂时无法载入此面板。
sidebar-retry = 重试
sidebar-graph-select = 选择节点查看详情。
sidebar-graph-open = 打开文献
matrix-scope-current = 当前文献
matrix-title = 标注矩阵
...
graph-roles-firstlast = 首位+末位
graph-roles-firstlast-tip = 按作者位次展示第一位和最后一位，不代表已确认通讯作者
...
tags-rename-invalid = 无法安全地依次重命名这些标签，请更换分支名称或匹配规则。
graph-authors-complete = 补全作者身份
graph-authors-complete-tip = 从 OpenAlex 获取当前范围缺失的作者身份（每次最多 30 篇）。
graph-authors-loading = 正在补全作者身份…
graph-authors-progress = 作者身份：已补全 { $updated } 篇 · 已请求 { $attempted } 篇
graph-authors-done = 作者身份补全结束：已补全 { $updated } 篇 · 已请求 { $attempted } 篇
graph-authors-stopped = 作者身份补全已停止：已补全 { $updated } 篇 · 已请求 { $attempted } 篇，请稍后重试
