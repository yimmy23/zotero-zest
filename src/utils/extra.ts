 ...
 * first paragraph and silently drop the rest, so these helpers treat a key as
 ...
 * openers are not mistaken for keys.
 */
const BLOCK_KEY_LINE =
  /^[ \t]*([A-Za-z_][A-Za-z0-9._\u4e00-\u9fff-]{0,39}(?: [A-Za-z0-9._\u4e00-\u9fff-]{1,20})?)[ \t]*:(.*)$/;

const isBlockKeyLine = (line: string) => BLOCK_KEY_LINE.test(line);

/** Read a possibly multi-line Extra value (first match wins, case-insensitive). */
export function getExtraBlockText(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const buf = [m[2].trim()];
    for (let j = i + 1; j < lines.length && !isBlockKeyLine(lines[j]); j++) {
      buf.push(lines[j]);
    }
...
/**
 * Compute the new Extra text with a block-valued `key` upserted (value === null
 * removes it). Pure; returns null when nothing changes. Multi-line values are
 * written as `key: firstLine` followed by the remaining lines verbatim, which
 * is exactly the shape the block reader recognises.
 */
export function upsertExtraBlockText(
  extra: string,
  keys: string[],
  value: string | null,
): string | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(BLOCK_KEY_LINE);
    const hit = !!m && wanted.includes(m[1].trim().toLowerCase());
    if (!hit) {
      out.push(lines[i]);
      continue;
    }
    // swallow this block's continuation lines whether we keep it or not
    const block = [lines[i]];
    while (i + 1 < lines.length && !isBlockKeyLine(lines[i + 1])) {
      block.push(lines[++i]);
    }
    if (done && m![1].trim().toLowerCase() !== writtenKey) {
      // a different spelling of the same concept is somebody else's line
      out.push(...block);
      continue;
    }
    if (done) {
      changed = true; // duplicate of the key we already wrote — drop it
      continue;
    }
    done = true;
    writtenKey = m![1].trim().toLowerCase();
    if (value === null) {
      changed = true;
      continue;
    }
    const next = `${m![1].trim()}: ${value}`.split("\n");
    if (next.length !== block.length || next.some((l, k) => l !== block[k])) {
      changed = true;
    }
    out.push(...next);
  }
  if (!done && value !== null) {
    while (out.length && out[out.length - 1].trim() === "") out.pop();
    out.push(...`${keys[0]}: ${value}`.split("\n"));
    changed = true;
  }
  return changed ? out.join("\n") : null;
}

/** Upsert one block-valued Extra key and save (skips the write when unchanged). */
export async function setExtraBlock(
  item: Zotero.Item,
  keys: string[],
  value: string | null,
): Promise<boolean> {
  ...
  const next = upsertExtraBlockText(extra, keys, value);
  if (next === null) return false;
  item.setField("extra", next);
  await item.saveTx({ skipSelect: true } as any);
  return true;
